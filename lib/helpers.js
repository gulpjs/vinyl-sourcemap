...
function contentIncluded(file, state, options, callback) {
  ...
  if (state.destPath == undefined) {
    ...
    // TODO: remove support for charset or implement in convert-source-map and use their .toComment()
    comment = commentFormatter(file, 'data:application/json;charset=' + options.charset + ';base64,' + base64Map);
  } else {
    var mapFile = path.join(state.destPath, file.relative) + '.map';
    // custom map file name
    // TODO: move function-option resolution from here
    ...
    if (options.sourceMappingURL && typeof options.sourceMappingURL === 'function') {
      comment = commentFormatter(file, options.sourceMappingURL(file));
    }
  }
