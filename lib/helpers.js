...
function writeSourceMaps(file, destPath, callback) {
  ...
  file.contents = appendBuffer(file.contents, new Buffer(comment));
...
function normalizePath(fp) {
  // A win32 path is forbidden to use '/' in a path element.
  var isWin32Path = !fp.includes('/') && fp.includes('\\');
  if (isWin32Path) {
    var parsed = path.win32.parse(fp);
    fp = fp.replace(/\\/g, '/');
    if (parsed.root && !fp.startsWith('/')) {
      fp = '/' + fp;
    }
  }
  return path.posix.normalize(fp);
}
