...
test('add: should return an error when no valid vinyl file is provided', function (t) {
	[
		{ type: 'null', val: null },
		{ type: 'an object', val: {} },
		...
		sourcemaps.add(obj.val, function(err, file) {
			t.ok(err instanceof Error && err.message === 'vinyl-sourcemap-add: Not a vinyl file', util.format('should not accept %s as first argument', obj.type));
		});
	});
	t.end();
});

test('add: should return an error when invalid options are provided', function (t) {
	var file = makeFile();
	[
		{ type: 'undefined' },
		{ type: 'null', val: null },
...
test('add: should add an empty sourcemap', function (t) {
...
test('add: should add a valid sourcemap if wished', function (t) {
...
test('add: should add a valid source map for CSS if wished', function (t) {
...
test('add: should import an existing inline source map', function (t) {
...
test('add: should remove inline sourcemap', function (t) {
	sourcemaps.add(makeFileWithInlineSourceMap(), { loadMaps: true }, function(err, data) {
...
test('add: should load external source map file referenced in comment with the \/\/# syntax', function (t) {
	var file = makeFile();
...
test('add: should remove source map comment with the \/\/# syntax', function (t) {
	var file = makeFile();
	file.contents = new Buffer(sourceContent + '\n//# sourceMappingURL=helloworld2.js.map');
	sourcemaps.add(file, { loadMaps: true }, function(err, data) {
		t.notOk(/sourceMappingURL/.test(data.contents.toString()), 'should not have sourcemapping');
...
test('add: should load external source map if no source mapping comment', function (t) {
	var file = makeFile();
...
test('add: should load external source map and add sourceContent if missing', function (t) {
	var file = makeFile();
...
test('add: should use unix style paths in sourcemap', function (t) {
	var file = makeFile();
	file.base = file.cwd;
...
test('add: should use sourceRoot when resolving path to sources', function (t) {
	var file = makeFile();
...
test('add: should not load source content if the path is a url', function (t) {
	var file = makeFile();
